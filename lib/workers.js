...
workers.validateCheckData = function(originalCheckData) {
  ...
  originalCheckData.lastChecked = typeof(originalCheckData.lastChecked) == 'number' && originalCheckData.lastChecked > 0 ? originalCheckData : false;
  ...
  var msg = 'Alert! Your check for: ' + newCheckData.method.toUpperCase() + ' '
    ...
    helpers.sendTwilioSms(newCheckData.userPhone, msg, function(error) {
      if(!error) {
        ...
        console.log('Could not send SMS alert to user with state change in their check');
      }
    });
};
